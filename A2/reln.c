...
// This is group work, aims for task3
// Last modified by
// Pan Luo: z5192086,
...
void splitReln(Reln r)
{
  ...
  r->npages++;
  Count num_t = 0;
  Tuple *tuple_stack = malloc(1024 * sizeof(Tuple));
  Offset current_tuple = 0;
  PageID pid = r->sp;
  FILE * file = r->data;
  int index = 0;
  while (pid != NO_PAGE) {
    ...
    Tuple tup_sting = read_next_tuple(file, pid, current_tuple);
    Bits hash = tupleHash(r, tup_sting);
    Bits new_pg_id = getLower(hash, r->depth + 1);
    if (new_pg_id == pid) {
        tuple_stack[index++] = copyString(tup_sting);
	}
    else {
    add_to_split_page(r, tup_sting, new_pg_id);
	}
    current_tuple += strlen(tup_sting)+1;
    num_t++;
    if (num_t >= pageNTuples(pg)) {
    Page cover = newPage();
  ...
  int i;
  for (i = 0; i < index; ++i) {
    add_to_split_page(r, tuple_stack[i], r->sp);
  }
...
Tuple read_next_tuple(FILE *in, PageID pid, Offset curtup){
 char line[MAXTUPLEN];
 fseek(in, PAGESIZE * pid  + 2*sizeof(Offset) + sizeof(Count) + curtup ,SEEK_SET);
